...
class AccessibilityAnalyzer {
  ...
  async checkVisualAccessibility($, page, visualAnalysis) {
    try {
      ...
      } catch (error) { console.warn('Error checking ARIA landmarks:', error.message); }
    ...
    try {
      ...
      if (videos.length > 0) {
        ...
        videos.each((i, video) => {
          ...
          } catch (error) { console.warn('Error checking individual video:', error.message); }
        });
        ...
        const keyboardIssues = await page.evaluate(() => {
          ...
          interactiveElements.forEach(el => {
            try {
              ...
              if (tabIndex === '-1' && !el.disabled) nonKeyboardAccessible++;
            } catch (e) { /* skip */ }
          });
      ...
      if (forms.length > 0) {
        ...
        forms.each((i, form) => {
          try {
            ...
            if (complexInputs > 0 && !hasHelp) formsWithoutHelp++;
          } catch (error) { console.warn('Error checking individual form:', error.message); }
        });
    ...
    try {
      ...
      inputs.each((i, input) => {
        try {
          ...
          let hasLabel = false;
          if (id) {
            try { hasLabel = $(`label[for="${id}"]`).length > 0; } catch (labelError) { console.warn('Error checking label:', labelError.message); }
          }
  ...
  calculateSummary(results) {
    ...
    try {
      Object.values(results.checks).forEach(check => {
        ...
        if (check.passed !== undefined) passedChecks += check.passed;
      });
